...
const Features = () => {
	...
	const listFeatures = features.map(feat => (
		<div key={feat.id} className="flex flex-col items-center px-6 text-center">
			...
			<h2 className="mt-16 mb-4 text-lg font-medium">{feat[0]}</h2>
			<p className="text-gray-400 mt-4 mb-6">{feat[1]}</p>
			<a className="text-sm font-medium text-blue-primary" href="#">{feat[2]}</a>
		</div>
	))
